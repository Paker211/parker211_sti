...
if [ $matches -gt 1 ] ; then
  ...
  do
    ...
    if [ -d $name] ; then
      ...
      size="$(ls -sdkl $nmae | awk '{print $1}' )"
      echo " $index)  $1  (size = ${size}Kb, deleted = $datename )"
    fi
    index=$(( $index + 1 ))
  done
  echo ""
  ...
  if [ -e "$dest/$1" ] ; then
    echo "\"$1\" already exist in this directory. Cannot overwrite." >&2
    exit 1
  fi
